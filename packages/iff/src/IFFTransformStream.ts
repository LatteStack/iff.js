...
export class IFFTransformStream<T = any> extends TransformStream<Uint8Array, T> {
  ...
  constructor(private readonly transformer?: Transformer<Chunk, T>) {
    super({
      ...transformer,
      transform: (_chunk, controller) => this.transform(_chunk, controller),
    })
  }
  ...
  transform = async (
    ...
    while (this.buffer.length >= HEADER_LENGTH) {
      ...
      await this.transformer?.transform?.(chunk, controller)
